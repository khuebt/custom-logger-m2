 ...
 * Log handler for
 */
class EndpointHandler extends \Monolog\Handler\StreamHandler
{

    const LOG_PATH = BP.'/var/log/api/';
    const CUSTOM_FORMAT = "%datetime%|%session_id%|%context%\n";

    public $fileName = BP.'/var/log/api/endpoint.log';
    ...
    public function __construct($logName, $format = self::CUSTOM_FORMAT)
    {
        ...
        $this->fileName = self::LOG_PATH.$servername.$logName.'_endpoint_'.$thaidate.'.log';
